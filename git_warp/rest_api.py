...
from git_warp.backend import GitWarp, ErrorResponse, GitError, GitWarpError, InvalidCommit, _LOG_PATH
...
def create_app(repo_path, token, log_path=None):
    ...
    gh = GitWarp(repo_path, log_path=log_path or _LOG_PATH)
    ...
    def api_rebase_split():
        body = get_body()
        return state_with_diff(gh.split(body.get("commit_hash", ""), body.get("files_to_split") or []), body.get("select_index"))
